const en = {
  categories: 'Categories',
  addCategory: 'Add Category',
  somethingIsWrong: 'Something is wrong',
  keepPrevious: 'I will keep the previous one',
  chooseActivity: 'Choose the activity',
  chooseCategory: 'Choose the category',
  chooseIcon: 'Choose the icon',
  name: 'Name',
  icon: 'Icon',
  save: 'Save',
  actionOptional: 'Action (optional)',
  times: 'times',
  hours: 'hours',
  perWeekTo: 'per week to',
  wantToDedicate: 'You want to dedicate',
  delete: 'Delete',
  confirmDelete: 'Are you sure you want to delete it?',
  okNo: 'Okay, nope',
  yes: 'Yes',
  username: 'Username',
  password: 'Password',
  login: 'Login',
  focusOn: 'Focus on',
  weeklyGoals: 'Weekly goals',
  pomodoro: 'Pomodoro',
  editGoal: 'Edit Goal',
  addGoal: 'Add Goal',
  editCategories: 'Edit Categories',
  editEvents: 'Edit Events',
  editCategory: 'Edit Category',
  editEvent: 'Edit Event',
  addEvent: 'Add Event',
  register: 'Register',
  edit: 'Edit',
  goals: 'Goals',
  logout: 'Logout',
  noEvents: 'Swipe right to edit this category and add events',
  noCategories: 'Swipe right to edit and add a category',
  events: 'Events',
  questionnaires: 'Questionnaires',
  questionnaire: 'Questionnary',
  question: 'Question',
  startQuestionnaire: 'Start Questionnaire',
  loginFailed: 'Login failed miserably',
  authError: 'Authorization problem, try login again',
  noGoals: 'Add weekly goals and track them on this screen',
  noEventsAtAll: 'You need to have at least one event to add a goal',
  noEditCategories: 'Add categories to organize events',
  noEditEvents: 'Add events in this category and optionally associate an action to them. The action can be any URL that your phone can open',
  // Configuration screen
  configuration: 'Configuration',
  synchronization: 'Synchronization',
  sync_now: 'Sync Now',
  user_information: 'User Information',
  server_url: 'Server URL',
  encryption_key: 'Encryption Key',
  frequency: 'Frequency',
  week_start_day: 'Week start day',
  last_name: 'Last Name',
  questionnaire_url: 'Questionnaire URL',
  import_questionnaire: 'Import Questionnaire',
  existing_questionnaires: 'Existing Questionnaires',
  manual: 'Manual',
  auto: 'Auto',
  never: 'Never',
  sunday: 'Sunday',
  monday: 'Monday',
  email: 'Email',
  pomodoro_duration: 'Duration (minutes)',
  server_url_placeholder: 'https://example.com',
  questionnaire_url_placeholder: 'https://...',
  checking_connection: 'Checking connection...',
  check_connection: 'Check connection',
  connection_ok: 'Connection OK',
  user_migrated: 'User migrated',
  check_connection_failed: 'Check connection failed',
  sync_in_progress: 'Sync in progress',
  syncing: 'Syncing...',
  last_sync: 'Last sync',
  sync_error: 'Sync error',
  sync_successful: 'Sync successful',
  sync_failed: 'Sync failed',
  settings_saved: 'Settings saved',
  sync_configuration_error: 'Sync configuration error',
}

const es = {
  categories: 'Categorías',
  addCategory: 'Añadir categoría',
  somethingIsWrong: 'Algo está mal',
  keepPrevious: 'Voy a mantener el anterior',
  chooseActivity: 'Elige la actividad',
  chooseCategory: 'Elige la categoría',
  chooseIcon: 'Elige el icono',
  name: 'Nombre',
  icon: 'Icono',
  save: 'Guardar',
  actionOptional: 'Acción (opcional)',
  times: 'veces',
  hours: 'horas',
  perWeekTo: 'por semana a',
  wantToDedicate: 'Quieres dedicar',
  delete: 'Borrar',
  confirmDelete: 'Que la vas a borrar',
  okNo: 'Vale, no',
  yes: 'Sí',
  username: 'Nombre de usuario',
  password: 'Contraseña',
  login: 'Iniciar sesión',
  focusOn: 'Céntrate en',
  weeklyGoals: 'Metas semanales',
  pomodoro: 'Pomodoro',
  editGoal: 'Editar Meta',
  addGoal: 'Añadir Meta',
  editCategories: 'Editar Categorías',
  editEvents: 'Editar Eventos',
  editCategory: 'Editar Categoría',
  editEvent: 'Editar Evento',
  addEvent: 'Añadir Evento',
  register: 'Registrar',
  edit: 'Editar',
  goals: 'Metas',
  logout: 'Cerrar sesión',
  noEvents: 'Desliza a la derecha para editar esta categoría y añadir eventos',
  noCategories: 'Desliza a la derecha para editar y añadir una categoría',
  noGoals: 'Añade metas semanales y sigue el progreso en esta pantalla',
  events: 'Eventos',
  questionnaires: 'Cuestionarios',
  questionnaire: 'Cuestionario',
  question: 'Pregunta',
  startQuestionnaire: 'Comenzar cuestionario',
  loginFailed: 'El inicio de sesión falló miserablemente',
  authError: 'Problema de autorización, intente iniciar sesión nuevamente',
  noEventsAtAll: 'Necesitas tener al menos un evento para añadir una meta',
  noEditCategories: 'Añade categorias para organizar eventos',
  noEditEvents: 'Añade eventos en esta categoría y opcionalmente asocia una acción a ellos. La acción puede ser cualquier URL que tu teléfono pueda abrir',
  configuration: 'Configuración',
  synchronization: 'Sincronización',
  sync_now: 'Sincronizar ahora',
  user_information: 'Información del usuario',
  server_url: 'URL del servidor',
  encryption_key: 'Clave de cifrado',
  frequency: 'Frecuencia',
  week_start_day: 'Día de inicio de semana',
  last_name: 'Apellido',
  questionnaire_url: 'URL del cuestionario',
  import_questionnaire: 'Importar cuestionario',
  existing_questionnaires: 'Cuestionarios Existentes',
  manual: 'Manual',
  auto: 'Auto',
  never: 'Nunca',
  sunday: 'Domingo',
  monday: 'Lunes',
  email: 'Correo electrónico',
  pomodoro_duration: 'Duración (minutos)',
  server_url_placeholder: 'https://ejemplo.com',
  questionnaire_url_placeholder: 'https://...',
  sync_disabled: 'Sincronizacion deshabilitada',
  sync_enabled: 'Sincronizacion habilitada',
  checking_connection: 'Comprobando conexión...',
  check_connection: 'Comprobar conexión',
  connection_ok: 'Conexión correcta',
  user_migrated: 'Usuario migrado',
  check_connection_failed: 'Error al comprobar la conexión',
  sync_in_progress: 'Sincronización en curso',
  syncing: 'Sincronizando...',
  last_sync: 'Última sincronización',
  sync_error: 'Error de sincronización',
  sync_successful: 'Sincronización exitosa',
  sync_failed: 'Sincronización fallida',
  settings_saved: 'Ajustes guardados',
  sync_configuration_error: 'Error de configuración de sincronización',
}

const pt = {
  categories: 'Categorias',
  addCategory: 'Adicionar categoria',
  somethingIsWrong: 'Algo está errado',
  keepPrevious: 'Vou manter o anterior',
  chooseActivity: 'Escolha a atividade',
  chooseCategory: 'Escolha a categoria',
  chooseIcon: 'Escolha o ícone',
  name: 'Nome',
  icon: 'Ícone',
  save: 'Salvar',
  actionOptional: 'Ação (opcional)',
  times: 'vezes',
  hours: 'horas',
  perWeekTo: 'por semana para',
  wantToDedicate: 'Quer dedicar',
  delete: 'Deletar',
  confirmDelete: 'Tem certeza que quer apagar isso?',
  okNo: 'Ok, não',
  yes: 'Sim',
  username: 'Nome de usuário',
  password: 'Senha',
  login: 'Entrar',
  focusOn: 'Foco em',
  weeklyGoals: 'Metas semanais',
  pomodoro: 'Pomodoro',
  editGoal: 'Editar Meta',
  addGoal: 'Adicionar Meta',
  editCategories: 'Editar Categorias',
  editEvents: 'Editar Eventos',
  editCategory: 'Editar Categoria',
  editEvent: 'Editar Evento',
  addEvent: 'Adicionar Evento',
  register: 'Registrar',
  edit: 'Editar',
  goals: 'Metas',
  logout: 'Sair',
  noEvents: 'Deslize para a direita para editar esta categoria e adicionar eventos',
  noCategories: 'Deslize para a direita para editar e adicionar uma categoria',
  events: 'Eventos',
  questionnaires: 'Questionários',
  questionnaire: 'Questionário',
  question: 'Pergunta',
  startQuestionnaire: 'Começar questionário',
  loginFailed: 'O login falhou miseravelmente',
  authError: 'Problema de autorização, tente fazer login novamente',
  noGoals: 'Adicione metas semanais e acompanhe o progresso nesta tela',
  noEventsAtAll: 'Precisa ter pelo menos um evento para adicionar uma meta',
  noEditCategories: 'Adicione categorias para organizar eventos',
  noEditEvents: 'Adicione eventos nesta categoria e, opcionalmente, associe uma ação a eles. A ação pode ser qualquer URL que seu telefone possa abrir',
  configuration: 'Configuração',
  synchronization: 'Sincronização',
  sync_now: 'Sincronizar agora',
  user_information: 'Informações do usuário',
  server_url: 'URL do servidor',
  encryption_key: 'Chave de criptografia',
  frequency: 'Frequência',
  week_start_day: 'Início da semana',
  last_name: 'Sobrenome',
  questionnaire_url: 'URL do questionário',
  import_questionnaire: 'Importar questionário',
  existing_questionnaires: 'Questionários Existentes',
  manual: 'Manual',
  auto: 'Auto',
  never: 'Nunca',
  sunday: 'Domingo',
  monday: 'Segunda-feira',
  email: 'Email',
  pomodoro_duration: 'Duração (minutos)',
  server_url_placeholder: 'https://exemplo.com',
  questionnaire_url_placeholder: 'https://...',
  checking_connection: 'Verificando conexão...',
  check_connection: 'Verificar conexão',
  connection_ok: 'Conexão OK',
  user_migrated: 'Usuário migrado',
  check_connection_failed: 'Falha ao verificar a conexão',
  sync_in_progress: 'Sincronização em andamento',
  syncing: 'Sincronizando...',
  last_sync: 'Última sincronização',
  sync_error: 'Erro de sincronização',
  sync_successful: 'Sincronização bem-sucedida',
  sync_failed: 'Falha na sincronização',
  settings_saved: 'Configurações salvas',
  sync_configuration_error: 'Erro na configuração da sincronização',
}

const jp = {
  categories: 'カテゴリー',
  addCategory: 'カテゴリーを追加',
  somethingIsWrong: '何かが間違っている',
  keepPrevious: '前に保持します',
  chooseActivity: 'アクティビティを選択します',
  chooseCategory: 'カテゴリーを選択します',
  chooseIcon: 'アイコンを選択します',
  name: '名前',
  icon: 'アイコン',
  save: '保存',
  actionOptional: 'アクション（オプション）',
  times: '回',
  hours: '時間',
  perWeekTo: '週に',
  wantToDedicate: 'あなたは欲しいです',
  delete: '削除',
  confirmDelete: '削除してもよろしいですか？',
  okNo: 'はい、いいえ',
  yes: 'はい',
  username: 'ユーザー名',
  password: 'パスワード',
  login: 'ログイン',
  focusOn: 'フォーカス',
  weeklyGoals: '週間のゴール',
  pomodoro: 'ポモドーロ',
  editGoal: '目標を編集',
  addGoal: '目標を追加',
  editCategories: 'カテゴリーを編集',
  editEvents: 'イベントを編集',
  editCategory: 'カテゴリーを編集',
  editEvent: 'イベントを編集',
  addEvent: 'イベントを追加',
  register: '登録',
  edit: '編集',
  goals: '目標',
  logout: 'ログアウト',
  noEvents: '右にスライドしてカテゴリーを編集してイベントを追加します',
  noCategories: '右にスライドしてカテゴリーを編集してカテゴリーを追加します',
  events: 'イベント',
  questionnaires: 'アンケート',
  questionnaire: 'アンケート',
  question: '質問',
  startQuestionnaire: 'アンケートを開始',
  loginFailed: 'ログインに失敗しました',
  authError: '認証エラー、もう一度ログインしてください',
  noGoals: '週間のゴールを追加して、この画面で進捗を追跡してください',
  noEventsAtAll: '目標を追加するには、少なくとも1つのイベントが必要です',
  noEditCategories: 'イベントを整理するためにカテゴリを追加します',
  noEditEvents: 'このカテゴリにイベントを追加し、必要に応じてアクションを関連付けます。アクションは、電話が開くことができるURLである必要があります',
  configuration: '設定',
  synchronization: '同期',
  sync_now: '今すぐ同期',
  user_information: 'ユーザー情報',
  server_url: 'サーバーURL',
  encryption_key: '暗号鍵',
  frequency: '頻度',
  week_start_day: '週の開始日',
  last_name: '姓',
  questionnaire_url: 'アンケートURL',
  import_questionnaire: 'アンケートをインポート',
  existing_questionnaires: '既存のアンケート',
  manual: '手動',
  auto: '自動',
  never: 'しない',
  sunday: '日曜日',
  monday: '月曜日',
  email: 'メール',
  pomodoro_duration: '時間（分）',
  server_url_placeholder: 'https://example.com',
  questionnaire_url_placeholder: 'https://...',
  checking_connection: '接続を確認しています...',
  check_connection: '接続を確認',
  connection_ok: '接続は正常です',
  user_migrated: 'ユーザーを移行しました',
  check_connection_failed: '接続確認に失敗しました',
  sync_in_progress: '同期中',
  syncing: '同期しています...',
  last_sync: '最終同期',
  sync_error: '同期エラー',
  sync_successful: '同期成功',
  sync_failed: '同期に失敗しました',
  settings_saved: '設定を保存しました',
  sync_configuration_error: '同期設定エラー',
}

const zh = {
  categories: '类别',
  addCategory: '添加类别',
  somethingIsWrong: '有错误',
  keepPrevious: '保持前面',
  chooseActivity: '选择活动',
  chooseCategory: '选择类别',
  chooseIcon: '选择图标',
  name: '名称',
  icon: '图标',
  save: '保存',
  actionOptional: '行动（可选）',
  times: '次',
  hours: '小时',
  perWeekTo: '每周到',
  wantToDedicate: '你想要自由',
  delete: '删除',
  confirmDelete: '你确定要删除它吗？',
  okNo: '好，不',
  yes: '是',
  username: '用户名',
  password: '密码',
  login: '登录',
  focusOn: '焦点',
  weeklyGoals: '每周目标',
  pomodoro: 'Pomodoro',
  editGoal: '编辑目标',
  addGoal: '添加目标',
  editCategories: '编辑类别',
  editEvents: '编辑事件',
  editCategory: '编辑类别',
  editEvent: '编辑事件',
  addEvent: '添加事件',
  register: '注册',
  edit: '编辑',
  goals: '目标',
  logout: '注销',
  noEvents: '滑动右侧编辑类别并添加事件',
  noCategories: '滑动右侧编辑并添加类别',
  events: '事件',
  questionnaires: '调查问卷',
  questionnaire: '调查问卷',
  question: '题',
  startQuestionnaire: '开始调查问卷',
  loginFailed: '登录失败',
  authError: '授权问题，请重新登录',
  noGoals: '添加每周目标并在本屏幕上跟踪',
  noEventsAtAll: '您需要至少一个事件才能添加目标',
  noEditCategories: '添加类别以组织事件',
  noEditEvents: '在此类别中添加事件，必要时将其与操作关联。操作可以是您的电话可以打开的任何URL',
  configuration: '配置',
  synchronization: '同步',
  sync_now: '立即同步',
  user_information: '用户信息',
  server_url: '服务器URL',
  encryption_key: '加密密钥',
  frequency: '频率',
  week_start_day: '一周开始日',
  last_name: '姓',
  questionnaire_url: '问卷URL',
  import_questionnaire: '导入问卷',
  existing_questionnaires: '现有问卷',
  manual: '手动',
  auto: '自动',
  never: '从不',
  sunday: '星期日',
  monday: '星期一',
  email: '电子邮件',
  pomodoro_duration: '时长（分钟）',
  server_url_placeholder: 'https://example.com',
  questionnaire_url_placeholder: 'https://...',
  checking_connection: '正在检查连接...',
  check_connection: '检查连接',
  connection_ok: '连接正常',
  user_migrated: '用户已迁移',
  check_connection_failed: '检查连接失败',
  sync_in_progress: '正在同步',
  syncing: '正在同步...',
  last_sync: '上次同步',
  sync_error: '同步错误',
  sync_successful: '同步成功',
  sync_failed: '同步失败',
  settings_saved: '设置已保存',
  sync_configuration_error: '同步配置错误',
}

const ru = {
  categories: 'Категории',
  addCategory: 'Добавить категорию',
  somethingIsWrong: 'Что-то не так',
  keepPrevious: 'Я оставляю предыдущий',
  chooseActivity: 'Выберите деятельность',
  chooseCategory: 'Выберите категорию',
  chooseIcon: 'Выберите значок',
  name: 'Имя',
  icon: 'Значок',
  save: 'Сохранить',
  actionOptional: 'Действие (необязательно)',
  times: 'раз',
  hours: 'часов',
  perWeekTo: 'за неделю до',
  wantToDedicate: 'Я хочу делиться',
  delete: 'Удалить',
  confirmDelete: 'Вы уверены, что хотите удалить это?',
  okNo: 'Да, нет',
  yes: 'Да',
  username: 'Имя пользователя',
  password: 'Пароль',
  login: 'Войти',
  focusOn: 'Фокус',
  weeklyGoals: 'Еженедельные цели',
  pomodoro: 'Помодоро',
  editGoal: 'Редактировать цель',
  addGoal: 'Добавить цель',
  editCategories: 'Редактировать категории',
  editEvents: 'Редактировать события',
  editCategory: 'Редактировать категорию',
  editEvent: 'Редактировать событие',
  addEvent: 'Добавить событие',
  register: 'Зарегистрироваться',
  edit: 'Редактировать',
  goals: 'Цели',
  logout: 'Выйти',
  noEvents: 'Прокрутите вправо, чтобы редактировать эту категорию и добавить события',
  noCategories: 'Прокрутите вправо, чтобы редактировать и добавить категорию',
  events: 'События',
  questionnaires: 'Опросники',
  questionnaire: 'Опросник',
  question: 'Вопрос',
  startQuestionnaire: 'Начать опрос',
  loginFailed: 'Ошибка входа',
  authError: 'Ошибка авторизации, пожалуйста, войдите снова',
  noGoals: 'Добавьте еженедельные цели и отслеживайте их на этом экране',
  noEventsAtAll: 'Вам нужно иметь хотя бы одно событие, чтобы добавить цель',
  noEditCategories: 'Добавьте категории для организации событий',
    noEditEvents: 'Добавьте события в эту категорию и при необходимости свяжите их с действием. Действие может быть любым URL, который ваш телефон может открыть',
  configuration: 'Конфигурация',
  synchronization: 'Синхронизация',
  sync_now: 'Синхронизировать сейчас',
  user_information: 'Информация о пользователе',
  server_url: 'URL сервера',
  encryption_key: 'Ключ шифрования',
  frequency: 'Частота',
  week_start_day: 'Начало недели',
  last_name: 'Фамилия',
  questionnaire_url: 'URL опросника',
  import_questionnaire: 'Импорт опросника',
  existing_questionnaires: 'Существующие опросники',
  manual: 'Вручную',
  auto: 'Авто',
  never: 'Никогда',
  sunday: 'Воскресенье',
  monday: 'Понедельник',
  email: 'Эл. почта',
  pomodoro_duration: 'Продолжительность (минуты)',
  server_url_placeholder: 'https://example.com',
  questionnaire_url_placeholder: 'https://...',
  checking_connection: 'Проверка соединения...',
  check_connection: 'Проверить соединение',
  connection_ok: 'Соединение в порядке',
  user_migrated: 'Пользователь мигрирован',
  check_connection_failed: 'Не удалось проверить соединение',
  sync_in_progress: 'Синхронизация выполняется',
  syncing: 'Синхронизация...',
  last_sync: 'Последняя синхронизация',
  sync_error: 'Ошибка синхронизации',
  sync_successful: 'Синхронизация прошла успешно',
  sync_failed: 'Синхронизация не удалась',
  settings_saved: 'Настройки сохранены',
  sync_configuration_error: 'Ошибка конфигурации синхронизации',
}

const ph = {
  categories: 'Mga Kategorya',
  addCategory: 'Magdagdag ng Kategorya',
  somethingIsWrong: 'Mayroong isang mali',
  keepPrevious: 'Gusto ko bang maghintay sa nakaraan',
  chooseActivity: 'Pumili ng Aktibidad',
  chooseCategory: 'Pumili ng Kategorya',
  chooseIcon: 'Pumili ng Icon',
  name: 'Pangalan',
  icon: 'Icon',
  save: 'I-save',
  actionOptional: 'Aksyon (opsyonal)',
  times: 'mga oras',
  hours: 'oras',
  perWeekTo: 'bawat linggo sa',
  wantToDedicate: 'Nais mo bang dedikasyon',
  delete: 'Tanggalin',
  confirmDelete: 'Sigurado ka bang gusto mong tanggalin ito?',
  okNo: 'Okay, hindi',
  yes: 'Oo',
  username: 'Pangalan ng tagagamit',
  password: 'Password',
  login: 'Mag-login',
  focusOn: 'Magfokus sa',
  weeklyGoals: 'Mga Meta bawat linggo',
  pomodoro: 'Pomodoro',
  editGoal: 'I-edit ang Meta',
  addGoal: 'Magdagdag ng Meta',
  editCategories: 'I-edit ang Mga Kategorya',
  editEvents: 'I-edit ang Mga Event',
  editCategory: 'I-edit ang Kategorya',
  editEvent: 'I-edit ang Event',
  addEvent: 'Magdagdag ng Event',
  register: 'Mag-register',
  edit: 'I-edit',
  goals: 'Mga Meta',
  logout: 'Mag-logout',
  noEvents: 'I-swipe sa kanan upang i-edit ang kategorya at magdagdag ng mga event',
  noCategories: 'I-swipe sa kanan upang i-edit at magdagdag ng kategorya',
  events: 'Mga Event',
  questionnaires: 'Mga Questionnaire',
  questionnaire: 'Questionnaire',
  question: 'Tanong',
  startQuestionnaire: 'Simulan ang Questionnaire',
  loginFailed: 'Nabigo ang pag-login',
  authError: 'May error sa awtorisasyon, mangyaring mag-login muli',
  noGoals: 'Magdagdag ng lingguhang mga layunin at subaybayan ang mga ito sa screen na ito',
  noEventsAtAll: 'Kailangan mong magkaroon ng hindi bababa sa isang event upang magdagdag ng layunin',
  noEditCategories: 'Magdagdag ng mga kategorya upang organisahin ang mga event',
  noEditEvents: 'Magdagdag ng mga event sa kategoryang ito at kung kinakailangan ay i-associate ang mga ito sa isang aksyon. Ang aksyon ay maaaring anumang URL na maaaring buksan ng iyong telepono',
  configuration: 'Configuration',
  synchronization: 'Synchronization',
  sync_now: 'Sync Now',
  user_information: 'User Information',
  server_url: 'Server URL',
  encryption_key: 'Encryption Key',
  frequency: 'Frequency',
  week_start_day: 'Week start day',
  last_name: 'Last Name',
  questionnaire_url: 'Questionnaire URL',
  import_questionnaire: 'Import Questionnaire',
  existing_questionnaires: 'Mga umiiral na questionnaire',
  manual: 'Manual',
  auto: 'Auto',
  never: 'Never',
  sunday: 'Sunday',
  monday: 'Monday',
  email: 'Email',
  pomodoro_duration: 'Duration (minutes)',
  server_url_placeholder: 'https://example.com',
  questionnaire_url_placeholder: 'https://...',
  checking_connection: 'Sinusuri ang koneksyon...',
  check_connection: 'Suriin ang koneksyon',
  connection_ok: 'Maayos ang koneksyon',
  user_migrated: 'Na-migrate ang user',
  check_connection_failed: 'Nabigo ang pagsuri ng koneksyon',
  sync_in_progress: 'Isinasagawa ang pagsi-sync',
  syncing: 'Nagsi-sync...',
  last_sync: 'Huling sync',
  sync_error: 'Error sa sync',
  sync_successful: 'Matagumpay ang sync',
  sync_failed: 'Nabigo ang sync',
  settings_saved: 'Na-save ang mga setting',
  sync_configuration_error: 'Error sa config ng sync',
}

const de = {
  categories: 'Kategorien',
  addCategory: 'Kategorie hinzufügen',
  somethingIsWrong: 'Irgendetwas ist schief gelaufen',
  keepPrevious: 'Ich werde die vorherige behalten',
  chooseActivity: 'Wählen Sie die Aktivität',
  chooseCategory: 'Wählen Sie die Kategorie',
  chooseIcon: 'Wählen Sie das Symbol',
  name: 'Name',
  icon: 'Symbol',
  save: 'Speichern',
  actionOptional: 'Aktion (optional)',
  times: 'mal',
  hours: 'Stunden',
  perWeekTo: 'pro Woche zu',
  wantToDedicate: 'Möchten Sie zu dedizieren',
  delete: 'Löschen',
  confirmDelete: 'Sind Sie sicher, dass Sie es löschen möchten?',
  okNo: 'Okay, nein',
  yes: 'Ja',
  username: 'Benutzername',
  password: 'Passwort',
  login: 'Anmelden',
  focusOn: 'Fokus auf',
  weeklyGoals: 'Wöchentliche Ziele',
  pomodoro: 'Pomodoro',
  editGoal: 'Ziel bearbeiten',
  addGoal: 'Ziel hinzufügen',
  editCategories: 'Kategorien bearbeiten',
  editEvents: 'Events bearbeiten',
  editCategory: 'Kategorie bearbeiten',
  editEvent: 'Event bearbeiten',
  addEvent: 'Event hinzufügen',
  register: 'Registrieren',
  edit: 'Bearbeiten',
  goals: 'Ziele',
  logout: 'Ausloggen',
  noEvents: 'Ziehen Sie nach rechts, um die Kategorie zu bearbeiten und Events hinzuzufügen',
  noCategories: 'Ziehen Sie nach rechts, um zu bearbeiten und Kategorien hinzuzufügen',
  events: 'Events',
  questionnaires: 'Fragebögen',
  questionnaire: 'Fragebogen',
  question: 'Frage',
  startQuestionnaire: 'Fragebogen starten',
  loginFailed: 'Anmeldung fehlgeschlagen',
  authError: 'Authentifizierungsfehler, bitte melden Sie sich erneut an',
  noGoals: 'Fügen Sie wöchentliche Ziele hinzu und verfolgen Sie diese auf diesem Bildschirm',
  noEventsAtAll: 'Sie müssen mindestens ein Event haben, um ein Ziel hinzuzufügen',
  noEditCategories: 'Fügen Sie Kategorien hinzu, um Events zu organisieren',
  noEditEvents: 'Fügen Sie Events in dieser Kategorie hinzu und verknüpfen Sie sie optional mit einer Aktion. Die Aktion kann eine beliebige URL sein, die Ihr Telefon öffnen kann',
  configuration: 'Konfiguration',
  synchronization: 'Synchronisierung',
  sync_now: 'Jetzt synchronisieren',
  user_information: 'Benutzerinformationen',
  server_url: 'Server-URL',
  encryption_key: 'Verschlüsselungsschlüssel',
  frequency: 'Frequenz',
  week_start_day: 'Wochenanfang',
  last_name: 'Nachname',
  questionnaire_url: 'Fragebogen-URL',
  import_questionnaire: 'Fragebogen importieren',
  existing_questionnaires: 'Vorhandene Fragebögen',
  manual: 'Manuell',
  auto: 'Auto',
  never: 'Nie',
  sunday: 'Sonntag',
  monday: 'Montag',
  email: 'E-Mail',
  pomodoro_duration: 'Dauer (Minuten)',
  server_url_placeholder: 'https://beispiel.com',
  questionnaire_url_placeholder: 'https://...',
  checking_connection: 'Verbindung wird überprüft...',
  check_connection: 'Verbindung prüfen',
  connection_ok: 'Verbindung OK',
  user_migrated: 'Benutzer migriert',
  check_connection_failed: 'Verbindungsprüfung fehlgeschlagen',
  sync_in_progress: 'Synchronisierung läuft',
  syncing: 'Wird synchronisiert...',
  last_sync: 'Letzte Synchronisierung',
  sync_error: 'Synchronisierungsfehler',
  sync_successful: 'Synchronisierung erfolgreich',
  sync_failed: 'Synchronisierung fehlgeschlagen',
  settings_saved: 'Einstellungen gespeichert',
  sync_configuration_error: 'Fehler in der Synchronisierungskonfiguration',
}

export { en, es, pt, jp, zh, ru, ph, de }
