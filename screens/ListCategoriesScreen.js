...
function ListCategoriesScreen({ navigation, categories }) {
    ...
    navigation.setOptions({
      headerRight: () => (
        <TouchableOpacity
          ...
          onPress={() => RootNavigation.navigate('Metas', { screen: 'ShowGoals' })}
        >
  ...
  const goToEvents = (item) =>
    RootNavigation.navigate('ListEvents', { category_id: item.id, category_name: item.name })

  const renderItem = ({ item }) => <Element element={item} onPress={() => goToEvents(item)} />
